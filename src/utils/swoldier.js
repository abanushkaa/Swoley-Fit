...
export function generateWorkout({ poison, muscles, goal }) {
  // Basic placeholder logic
  const selected = WORKOUTS[poison];
  
  if (!selected) return [];

  // Flatten nested categories if necessary
  let allMuscles = Array.isArray(selected)
    ? selected
    : Object.values(selected).flat();

  // Filter by user-selected muscles
  let filtered = allMuscles.filter(m => muscles.includes(m));

  // Return a mock workout (replace with real logic later)
  return {
    goal,
    type: poison,
    muscles: filtered.length > 0 ? filtered : allMuscles.slice(0, 3),
